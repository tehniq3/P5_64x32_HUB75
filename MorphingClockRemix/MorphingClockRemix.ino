...
void draw_weather ()
{
  ...
  else
  {
    ...
    lstr = " " + String (humiM) + "%";
    xo = 12*TF_COLS;
    if (numar % 2 == 0) TFDrawText (&display, lstr, xo, yo, lcc);
    //-pressure
    lstr = String (presM)+"P";
    xo = 12*TF_COLS;
    if (numar % 2 != 0) TFDrawText (&display, lstr, xo, yo, cc_red);
    numar = numar + 1;
    ...
    */
    ...
    if (tempMax > -10000)
    {
      ...
      if (abs(tempMax) > 99)
        xo = 10*TF_COLS;
       */
       if (tempMax < 0) xo = xo-4;
       if (abs(tempMax) >= 10) xo = xo-4;
  ...
  for (int i = 0; i < 5; i += 2)
  {
    switch (date_fmt[i])
    {
      ...
      case 'M':
        ...
        break;
      case 'Y':
        lstr += String(abs(year(tnow)-2000));
        if (i < 4)
          lstr += date_fmt[i + 1];
        break;
    }
...
//#define SHOW_SOME_LOVE
 ...
 //weather conditions
  ...
  int  lcc = cc_red;
    ...
    xo = 12*TF_COLS; yo = 0;
    if (millis()/3000 % 2 == 0) TFDrawText (&display, lstr, xo, yo, lcc);
    //-pressure
    lstr = String (presM)+"P";
    xo = 12*TF_COLS;
    if (millis()/3000 % 2 != 0) TFDrawText (&display, lstr, xo, yo, cc_ylw);
  //
	//delay (0);
}
