...
void wifi_setup ()
{
  ...
  display.setTextColor (display.color565 (0, 0, 255));
    ...
    TFDrawText (&display, String(" MORPHING CLOCK "), 0, 13, display.color565(0, 0, 255));
    //TFDrawText (&display, String(" REMIX ver. 1.h "), 0, 20, display.color565(255, 255, 0));
    TFDrawText (&display, String("niq_ro's rem~ix "), 0, 20, display.color565(255, 255, 0));
    delay(3000);
    display.fillScreen (display.color565 (0, 0, 0));
...
*/
    display.fillScreen (display.color565 (0, 0, 0));
    TFDrawText (&display, String("AP:"), 0, 0, display.color565(0, 0, 255));
    TFDrawText (&display, String(wifiManagerAPName), 15, 0, display.color565(255, 0, 255));
    TFDrawText (&display, String("Pw:"), 0, 10, display.color565(0, 0, 255));
    TFDrawText (&display, String(wifiManagerAPPassword), 15, 10, display.color565(255, 0, 255));
    TFDrawText (&display, String("IP:"), 0, 20, display.color565(0, 0, 255));
    TFDrawText (&display, String("192.168.4.1"), 10, 20, display.color565(255, 0, 255));
...
String apiKey   = "inca2berirecepentruNicu"; //e.g a hex string like "abcdef0123456789abcdef0123456789"
...
/* 
  //draw the first 5 letters from the unknown weather condition
  String lstr = condS.substring (0, (condS.length () > 8?8:condS.length ()));
  lstr.toUpperCase ();
  TFDrawText (&display, lstr, x1, yo, cc_grn);
*/
String lstr = "";
if (lang1 == 0)
{
...
{
 if (condM != 0) draw_weather_conditions (); //weather conditions
 if (ics == 0) draw_date();
 if ((ics == 1) and (tempM > -100)) show_temp(); 
 if ((ics == 2) and (humiM >0)) show_humi();
 if ((ics == 3) and (presM >0)) show_pres();
 if ((ics == 4) and (wind > -1)) show_wind();
 if (ics == 5) show_zi();
